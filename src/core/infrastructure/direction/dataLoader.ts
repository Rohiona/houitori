...
import type { BoardData, CompatibilityTable } from "@/core/domain/direction";
...
export function loadBoardData(year: number): BoardData {
  const filePath = path.join(process.cwd(), "src/core/infrastructure/data/boards", `${year}.json`);
  try {
...
export function getCompatibilityTable(): CompatibilityTable {
  const filePath = path.join(process.cwd(), "src/core/infrastructure/data/compatibility.json");
  const content = fs.readFileSync(filePath, "utf-8");
